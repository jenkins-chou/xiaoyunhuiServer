...
router.post('/getScoreListByMatchId',function (req, res) {
    ...
    var sql = "SELECT a.*,b.* from user a,score b where a.user_id = any(select user_id from score where match_id = "+match_id+") and a.user_id = b.user_id and b.match_id = "+match_id+" and b.score_del != 'delete'";
    connectDB.query(sql,function(result){
        console.log(result);
        return res.jsonp(result);
    });
...
router.post('/getAllScoreIntegral', function (req, res) {
    var sql = "select a.score_value,a.score_unit,a.score_integral,b.user_name,d.team_name,e.match_title,e.match_time,e.match_address from score a,user b,user_team c,team d,matchs e where a.user_id = b.user_id and b.user_id = c.user_id and c.team_id = d.team_id and a.match_id = e.match_id and a.score_del != 'delete' and b.user_del != 'delete' and c.user_team_del != 'delete' and d.team_del != 'delete' and e.match_del !='delete' order by d.team_name";
    connectDB.query(sql,function(result){
            console.log(result);
            return res.jsonp(result);
        })
